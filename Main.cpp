...
void DataDummyNasabah() {
    // Tambah Nasabah Dummy 1
    akun* n1 = new akun;
    n1->data.nama = "Ahmad Fauzi";
    n1->data.pin = 1234;
    n1->data.username = "ahmad123";
    n1->data.password = "passahmad";
    n1->data.norek = 1029384756;
    n1->data.saldo = 750000;
    n1->data.isblokir = false;
    n1->data.totalMutasi = 0;
    catatLogTransaksi(&(n1->data), "MASUK", "Setoran Saldo Pembuatan Akun", 750000);
    n1->next = head;
    head = n1;
    totalnasabah++;

    // Tambah Nasabah Dummy 2
    akun* n2 = new akun;
    n2->data.nama = "Rian Hidayat";
    n2->data.pin = 8888;
    n2->data.username = "rian_hd";
    n2->data.password = "passrian";
    n2->data.norek = 5544221133;
    n2->data.saldo = 24500000;
    n2->data.isblokir = false;
    n2->data.totalMutasi = 0;
    catatLogTransaksi(&(n2->data), "MASUK", "Penerimaan Kliring Payroll", 24500000);
    n2->next = head;
    head = n2;
    totalnasabah++;
}
...
void lihatdatanasabah() {
    bersihkanLayar();
    cout << "======================================\n";
    cout << "     DATA MASTER REKENING NASABAH     \n";
    cout << "======================================\n";
    
    akun* temp = head;
    if (temp == nullptr) {
        cout << "Belum ada data nasabah di sistem.\n";
    }
    
    while (temp != nullptr) {
        string statusAkun = temp->data.isblokir ? (ALERT_RED + "TERBLOKIR") : (SUCCESS_GRN + "AKTIF");
        // Karakter » di bawah ini sudah diganti menjadi -
...
void kelolaakunnasabah() {
    bersihkanLayar();
    long long rekTarget;
    cout << "\nMasukkan Nomor Rekening Target: "; cin >> rekTarget;
    
    akun* target = carinasabahByRek(rekTarget);
    
    if (target != nullptr) {
        cout << "Pemilik Sah: " << GOLD_GOLD << target->data.nama << RESET << "\n\n";
        cout << "[1] Blokir Akun (Toggle Status)\n[2] Hapus Akun\n\nPilih tindakan: ";
        int opsiAksi; cin >> opsiAksi;
        
        if (opsiAksi == 1) {
            target->data.isblokir = !target->data.isblokir;
            cout << "\nStatus Blokir Akun Berhasil Diubah!\n";
        } else if (opsiAksi == 2) {
            // Logika Hapus Node Linked List
            if (head == target) {
                head = head->next;
            } else {
                akun* prev = head;
            ...
            delete target;
            totalnasabah--;
            cout << "\nAkun berhasil dihapus dari sistem.\n";
        }
    } else {
        cout << "Data nomor rekening tidak ditemukan.\n";
    }
}
...
int main () {
    ...
    menuUtama(); 
    return 0;
>>>>>>> 4bd48fd3ee81e77ac4726e19bcbcdf721cbe00af
}