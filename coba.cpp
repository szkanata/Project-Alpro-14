            ...
            if (kursor == 1) { 
                ...
                for (int i = 0; i < slotNasabahKini; i++) {
                    if (dataNasabah[i].apakahAktif) {
                        ...
                        cout << "             " << BLUE_PURE << "Â» " << RESET << TEXT_BLACK << "Nama: " << RESET << GOLD_GOLD << dataNasabah[i].nama << string(spacePad, ' ') << RESET 
    ...
    while (true) {
        ...
        if (tombol == 0 || tombol == -32) {
            ...
            if (kursor == 1) { 
                ...
                bersihkanLayar(); long long nominal;
                cout << "\n                    " << BLUE_PURE << "[CASH OUTLET DISPENSER]" << RESET << "\n";
                cout << "                    Masukkan Nominal Penarikan : Rp "; cin >> nominal;
                
                if (nominal <= 0 || nominal % 50000 != 0) {
                    cout << "\n                    " << ALERT_RED << "[TRANSAKSI GAGAL] Penarikan harus kelipatan Rp 50.000!" << RESET << "\n";
                } else if (nominal > ptrUser->saldo) {
                    cout << "\n                    " << ALERT_RED << "[TRANSAKSI GAGAL] Saldo Utama Rekening Anda Tidak Mencukupi!" << RESET << "\n";
                } else {
                    ptrUser->saldo -= nominal; 
                    catatLogTransaksi(ptrUser, "KELUAR", "Penarikan Tunai Mandiri", nominal);
                    cout << "\n                    " << SUCCESS_GRN << "[NOTIFIKASI BERHASIL] Silakan Ambil Uang Tunai Anda." << RESET << "\n";
                }
                tungguEnter();
            }
            else if (kursor == 3) { 
